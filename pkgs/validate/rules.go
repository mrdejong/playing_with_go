// Package validate provides a set of rules for validating fields.
// Stolen from https://github.com/anthdm/superkit/blob/master/validate
package validate
...
var Required = RuleSet{
	...
	ValidateFunc: func(rule RuleSet) bool {
		str, ok := rule.FieldRawValue.(string)
		if !ok {
			return false
		}
		return len(str) > 0
...
func TimeAfter(t time.Time) RuleSet {
	return RuleSet{
		...
		MessageFunc: func(set RuleSet) string {
			return fmt.Sprintf("is not before %v", t)
		},
	}
}
...
func TimeBefore(t time.Time) RuleSet {
	return RuleSet{
		...
		MessageFunc: func(set RuleSet) string {
			return fmt.Sprintf("is not after %v", t)
		},
	}
}
...
func Age(n int) RuleSet {
	return RuleSet{
		...
		ValidateFunc: func(set RuleSet) bool {
			...
			now := time.Now()
			years := now.Year() - v.Year()
			birthday := getAdjustedBirthDay(v, now)
		...
		MessageFunc: func(set RuleSet) string {
			return fmt.Sprintf("minimum age is %v", n)
		},
	}
}
